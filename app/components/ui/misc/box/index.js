...
export const EmbloyBox = ({children, className, variant}) => {
    ...
    return (
        <div className={`border-[1px] border-etna  w-full flex landscape:flex-row portrait:flex-col items-start landscape:justify-between portrait:justify-start bg-transparent dark:bg-chianti landscape:px-6 portrait:px-4 landscape:py-6 portrait:py-5 rounded-lg dark:border-biferno border-etna ${appearance} ${className} `}>
            {children}
        </div>
    );
}
